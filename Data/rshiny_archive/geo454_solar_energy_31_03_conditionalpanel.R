...
ui <- fluidPage(theme = shinytheme("sandstone"),
  fluidRow(
    # three conditional panels which show depending on the radio button
    conditionalPanel(condition = "input.second_map == 'def'", leafletOutput("map")),
    conditionalPanel(condition = "input.second_map == 'pol'", 
                     fluidRow(
                       column(6, "pol"),
                       column(6, "pol"))),
    conditionalPanel(condition = "input.second_map == 'prop'", 
                     fluidRow(
                       column(6, "prop"),
                       column(6, "prop")))
  ),
  fluidRow(
    column(2,
           ...
           div(HTML("<b>Add to the plots</b>"), style="font-size: 15px;"),
            ...
            radioButtons(inputId="second_map", label="Second map",
                         choices = c("Default" = "def", 
                                     "Political orientation" = "pol", 
                                     "Property ownership" = "prop"), selected = "def")
           ),
    column(5,plotOutput("plot")),
    column(5,"Static plot with socioeconomic variables")
  )
)
